...
module.exports = msgHandler = async (yakeys = new Client(), message) => {
    try {
        ...
        const botNumber = await yakeys.getHostNumber() + '@c.us'
        const blockNumber = await yakeys.getBlockedIds()
        const ownerNumber = config.ownerBot
        const authorWm = config.authorStick
        const packWm = config.packStick
        const groupId = isGroupMsg ? chat.groupMetadata.id : ''
        const groupAdmins = isGroupMsg ? await yakeys.getGroupAdmins(groupId) : ''
        ...
        if (isGroupMsg && !isGroupAdmins && isBotGroupAdmins && isDetectorOn && !isOwner) {
            if (chats.match(new RegExp(/(https:\/\/chat.whatsapp.com)/gi))) {
                const valid = await yakeys.inviteInfo(chats)
                if (valid) {
                    console.log(color('[KICK]', 'red'), color('Received a group link and it is a valid link!', 'yellow'))
                    await yakeys.reply(from, ind.linkDetected(), id)
                    await yakeys.removeParticipant(groupId, sender.id)
                } else {
        ...
        if (isGroupMsg && !isGroupAdmins && isBotGroupAdmins && !isOwner) {
           if (chats.length > 5000) {
               await yakeys.sendTextWithMentions(from, `@${sender.id} is detected sending a virtext.\nYou will be kicked!`)
               await yakeys.removeParticipant(groupId, sender.id)
            }
        } 
        ...
        if (isGroupMsg && isRegistered) {
            if (_stick.includes(chats)) {
                await yakeys.sendImageAsSticker(from, `./temp/sticker/${chats}.webp`, { author: authorWm, pack: packWm })
            }
        }
        ...
        if (isGroupMsg && !isGroupAdmins && isBotGroupAdmins && isDetectorOn && !isOwner) {
            if (chats.match(new RegExp(/(https:\/\/chat.(?!whatsapp.com))/gi))) {
                ...
                await yakeys.reply(from, 'Fake group link detected!', id)
                await yakeys.removeParticipant(groupId, sender.id)
            }
        }
            ...
            if (isUrl(chats)) {
                ...
                isPorn(classify.hostname, async (err, status) => {
                    ...
                    if (status) {
                        ...
                        await yakeys.reply(from, ind.linkNsfw(), id)
                        await yakeys.removeParticipant(groupId, sender.id)
                    } else {
        ...
        if (isGroupMsg && isAutoStickerOn && isMedia && isImage && !isCmd) {
            ...
            await yakeys.sendImageAsSticker(from, imageBase64, { author: authorWm, pack: packWm })
        ...
        if (isGroupMsg && isAutoStickerOn && isMedia && isVideo && !isCmd) {
            ...
            await yakeys.sendMp4AsSticker(from, videoBase64, { stickerMetadata: true, pack: packWm, author: authorWm, fps: 30, startTime: '00:00:00.0', endTime : '00:00:05.0', crop: false, loop: 0 })
            console.log(`Sticker processed for ${processTime(t, moment())} seconds`)
        }
        ...
        if (isGroupMsg) {
            for (let ment of mentionedJidList) {
                if (afk.checkAfkUser(ment, _afk)) {
                    ...
                    await yakeys.reply(from, ind.afkMentioned(getReason, getTime), id)
                }
            }
            if (afk.checkAfkUser(sender.id, _afk) && !isCmd) {
                _afk.splice(afk.getAfkPosition(sender.id, _afk), 1)
                fs.writeFileSync('./database/user/afk.json', JSON.stringify(_afk))
                await yakeys.sendText(from, ind.afkDone(pushname))
            }
        }
        ...
        switch (command) {
            case prefix+'antiporn': // Premium, chat VideFikri
                await yakeys.reply(from, 'Premium feature!\n\nContact: wa.me/6283803284713?text=Buy%20Anti%20Porn', id)
            break

            // Register by YakkoXCode
            case prefix+'register':
                if (isRegistered) return await yakeys.reply(from, ind.registeredAlready(), id)
                if (isGroupMsg) return await yakeys.reply(from, ind.pcOnly(), id)
                if (!q.includes('|')) return await yakeys.reply(from, ind.wrongFormat(), id)
                const namaUser = q.substring(0, q.indexOf('|') - 1)
                const umurUser = q.substring(q.lastIndexOf('|') + 2)
                const serialUser = createSerial(20)
                if (Number(umurUser) >= 40) return await yakeys.reply(from, ind.ageOld(), id)
                register.addRegisteredUser(sender.id, namaUser, umurUser, time, serialUser, _registered)
                await yakeys.reply(from, ind.registered(namaUser, umurUser, sender.id, time, serialUser), id)
                console.log(color('[REGISTER]'), color(time, 'yellow'), 'Name:', color(namaUser, 'cyan'), 'Age:', color(umurUser, 'cyan'), 'Serial:', color(serialUser, 'cyan'))
            break

            // Level [BETA] by YakkoXCode
            case prefix+'level':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                if (!isLevelingOn) return await yakeys.reply(from, ind.levelingNotOn(), id)
                if (!isGroupMsg) return await yakeys.reply(from, ind.groupOnly(), id)
                const userLevel = level.getLevelingLevel(sender.id, _level)
                const userXp = level.getLevelingXp(sender.id, _level)
                const ppLink = await yakeys.getProfilePicFromServer(sender.id)
                if (ppLink === undefined) {
                    var pepe = errorImg
                } else {
                    pepe = ppLink
                ...
                rank.build()
                    .then(async (buffer) => {
                        ...
                        await yakeys.sendImage(from, imageBase64, 'rank.png', '', id)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await yakeys.reply(from, 'Error!', id)
                    })
            break
            case prefix+'leaderboard':
                ...
                try {
                    ...
                    await yakeys.reply(from, leaderboard, id)
                } catch (err) {
                    console.error(err)
                    await yakeys.reply(from, ind.minimalDb(), id)
                }
            break
            ...
            case prefix+'joox': // By Hafizh
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                if (!q) return await yakeys.reply(from, ind.wrongFormat(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await yakeys.reply(from, ind.wait(), id)
                ...
                cardJoox.build()
                    .then(async (buffer) => {
                        ...
                        await yakeys.sendFile(from, `${sender.id}_joox.png`, 'joox.png', ind.joox(dataJoox.data), id)
                        fs.unlinkSync(`${sender.id}_joox.png`)
                        await yakeys.sendFileFromUrl(from, dataJoox.data.result[0].linkMp3, 'joox.mp3', '', id)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await yakeys.reply(from, 'Error!', id)
            ...
            case prefix+'instadl':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                if (!isUrl(url) && !url.includes('instagram.com')) return await yakeys.reply(from, ind.wrongFormat(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await yakeys.reply(from, ind.wait(), id)
                    ...
                    .then(async ({ result }) => {
                        for (let i = 0; i < result.post.length; i++) {
                            if (result.post[i].type === 'image') {
                                await yakeys.sendFileFromUrl(from, result.post[i].urlDownload, 'igpostdl.jpg', `*...:* *Instagram Downloader* *:...*\n\nUsername: ${result.owner_username}\nCaption: ${result.caption}`, id)
                            } else if (result.post[i].type === 'video') {
                                await yakeys.sendFileFromUrl(from, result.post[i].urlDownload, 'igpostdl.mp4', `*...:* *Instagram Downloader* *:...*\n\nUsername: ${result.owner_username}\nCaption: ${result.caption}`, id)
                            }
                        }
            ...
            case prefix+'fb':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(pushname), id)
                if (!isUrl(url) && !url.includes('facebook.com')) return await yakeys.reply(from, ind.wrongFormat(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await yakeys.reply(from, ind.wait(), id)
                downloader.fb(url)
                    .then(async ({ result }) => {
                            await yakeys.sendFileFromUrl(from, result.VideoUrl, 'videofb.mp4', '', id)
            ...
            break
                case prefix+'ytmp3':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                if (!isUrl(url) && !url.includes('youtu.be')) return await yakeys.reply(from, ind.wrongFormat(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await yakeys.reply(from, ind.wait(), id)
                downloader.ytdl(url)
                    .then(async ({result}) => {
                        if (Number(result.size.split(' MB')[0]) >= 30) {
                            await yakeys.reply(from, ind.musiclimit(), id)
                        } else {
                            await yakeys.sendFileFromUrl(from, result.imgUrl, `${result.title}.jpg`, ind.ytFound(result), id)
                            await yakeys.sendFileFromUrl(from, result.UrlMp3, `${result.title}.mp3`, '', id)
            ...
            case prefix+'ytmp4':
                ...
                downloader.ytdl(url)
                    .then(async ({result}) => {
                        if (Number(result.size.split(' MB')[0]) >= 30) {
                            await yakeys.reply(from, ind.videoLimit(), id)
                        } else {
                            await yakeys.sendFileFromUrl(from, result.imgUrl, `${result.title}.jpg`, ind.ytFound(result), id)
                            await yakeys.sendFileFromUrl(from, result.UrlVideo, `${result.title}.mp3`, '', id)
                            console.log('Success sending YouTube video!')
                        }
                    })
                    .catch(async (err) => {
            ...
            case prefix+'tiktokpic':
                ...
                try {
                    ...
                    if (tkt.data.error) return yakeys.reply(from, tkt.data.error, id)
                    await yakeys.sendFileFromUrl(from, tkt.data.result, 'tiktokpic.jpg', 'Ini :D', id)
            ...
            case prefix+'tktnowm':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                if (!isUrl(url) && !url.includes('tiktok.com')) return await yakeys.reply(from, ind.wrongFormat(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await yakeys.reply(from, ind.wait(), id)
                downloader.tikNoWm(url)
                    .then(async ({ result }) => {
                        await yakeys.sendFileFromUrl(from, result.thumb, 'TiktokNoWM.jpg', `➸ *Username*: ${result.username}\n➸ *Caption*: ${result.caption}\n➸ *Uploaded on*: ${result.uploaded_on}\n\nSedang dikirim, sabar ya...`, id)
                        const responses = await fetch(result.link)
                        const buffer = await responses.buffer()
                        fs.writeFileSync(`./temp/${sender.id}_TikTokNoWm.mp4`, buffer)
                        await yakeys.sendFile(from, `./temp/${sender.id}_TikTokNoWm.mp4`, `${sender.id}_TikTokNoWm.mp4`, '', id)
            ...
            case prefix+'tiktok':
                ...
                downloader.tik(url)
                    .then(async ({ result })=> {
                        await yakeys.sendFileFromUrl(from, result.video, 'TikTok.mp4', '', id)
            ...
            case prefix+'twt':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                if (!isUrl(url) && !url.includes('twitter.com')) return await yakeys.reply(from, ind.wrongFormat(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await yakeys.reply(from, ind.wait(), id)
                downloader.tweet(url)
                    .then(async (data) => {
                        if (data.type === 'video') {
                            ...
                            await yakeys.sendFileFromUrl(from, content[0].url, 'video.mp4', `Link HD: ${result}`, id)
                            ...
                            for (let i = 0; i < data.variants.length; i++) {
                                await yakeys.sendFileFromUrl(from, data.variants[i], data.variants[i].split('/media/')[1], '', id)
                                .then(() => console.log('Success sending Twitter media!'))
                                .catch(async (err) => {
                                    console.error(err)
                                    await yakeys.reply(from, 'Error!', id)
            ...
            case prefix+'moddroid': // Chikaa Chantekkzz
                ...
                downloader.modroid(q)
                    .then(async ({ status, result }) => {
                        if (status !== 200) {
                            await yakeys.reply(from, 'Not found.', id)
                        } else {
                            await yakeys.sendFileFromUrl(from, result[0].image, 'ksk.jpg', `*「 MOD FOUND 」*\n\n➸ *APK*: ${result[0].title}\n\n➸ *Size*: ${result[0].size}\n➸ *Publisher*: ${result[0].publisher}\n➸ *Version*: ${result[0].latest_version}\n➸ *Genre*: ${result[0].genre}\n\n*Download link*\n${result[0].download}`, id)
            ...
            case prefix+'linedl': // chikaa chantexxzz
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                if (isGroupMsg) return await yakeys.reply(from, ind.pcOnly(), id)
                if (!isUrl(url) && !url.includes('store.line.me')) return await yakeys.reply(from, ind.wrongFormat(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                downloader.line(url)
                    .then(async (res) => {
                        await yakeys.sendFileFromUrl(from, res.thumb, 'line.png', `*「 LINE STICKER DOWNLOADER 」*\n\n➸ *Title*: ${res.title}\n➸ *Sticker type*: ${res.type}\n\n_Media sedang dikirim, mohon tunggu sebentar..._`, id)
                        for (let i = 0; i < res.sticker.length; i++) {
                            await yakeys.sendStickerfromUrl(from, `${res.sticker[i]}`, null, { author: authorWm, pack: packWm })
            ...
            case prefix+'ocr': // by: VideFrelan
                ...
                if (isMedia && isImage || isQuotedImage || isQuotedSticker) {
                    ...
                    ocrtess.recognize(`./temp/${sender.id}.jpg`, ocrconf)
                        .then(async (text) => {
                            await yakeys.reply(from, `*...:* *OCR RESULT* *:...*\n\n${text}`, id)
                    ...
                    await yakeys.reply(from, ind.wrongFormat(), id)
                }
            break
            ...
            case prefix+'googlesearch':
                ...
                google({ 'query': q, 'no-display': true })
                    .then(async (results) => {
                        ...
                        await yakeys.reply(from, txt, id)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await yakeys.reply(from, 'Error!', id)
            ...
            case prefix+'say':
                ...
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await yakeys.sendText(from, q)
            break
            case prefix+'afk': // by YakkoXCode
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                if (!isGroupMsg) return await yakeys.reply(from, ind.groupOnly(), id)
                if (isAfkOn) return await yakeys.reply(from, ind.afkOnAlready(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                const reason = q ? q : 'Nothing.'
                afk.addAfkUser(sender.id, time, reason, _afk)
                await yakeys.reply(from, ind.afkOn(pushname, reason), id)
            break
            case prefix+'lyric':
            case prefix+'lirik':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                ...
                misc.lirik(q)
                    .then(async ({ result }) => {
                        if (result.code !== 200) return await yakeys.reply(from, 'Not found.', id)
                        await yakeys.reply(from, result.result, id)
            ...
            case prefix+'shortlink':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                if (!isUrl(url)) return await yakeys.reply(from, ind.wrongFormat(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                const urlShort = await misc.shortener(url)
                await yakeys.reply(from, ind.wait(), id)
                await yakeys.reply(from, urlShort, id)
                console.log('Success!')
            break
            ...
            case prefix+'wiki':
                ...
                misc.wiki(q)
                    .then(async ({ result, status }) => {
                        if (status !== 200) {
                            return await yakeys.reply(from, 'Not found.', id)
                        } else {
                            await yakeys.reply(from, result, id)
                ...
                misc.wikien(q)
                    .then(async ( { result }) => {
                        if (result.status !== '200') {
                            ...
                            await yakeys.reply(from, `➸ *PageId*: ${result.pageid}\n➸ *Title*: ${result.title}\n➸ *Result*: ${result.desc}`, id)
                        }
                    })
                    .catch(async (err) => {
                        console.error(err)
            ...
            case prefix+'ttp': // CHIKAA CHANTEKKXXZZ
                ...
                misc.ttp(q)
                    .then(async (res) => {
                        await yakeys.sendImageAsSticker(from, res.base64, { author: authorWm, pack: packWm })
            ...
            case prefix+'genshin':
                ...
                try {
                    ...
                    await yakeys.sendFileFromUrl(from, character.image, `${character.name}.jpg`, `*「 GENSHIN IMPACT 」*\n\n*${character.name}*\n${character.description}\n\n"_${character.quote}_" - ${character.name}\n\n➸ *Name*: ${character.name}\n➸ *Seiyuu*: ${character.cv}\n➸ *Region*: ${character.city}\n➸ *Rating*: ${character.rating}\n➸ *Vision*: ${character.element}\n➸ *Weapon*: ${character.weapon}\n\n${character.url}`)
                    console.log('Success sending Genshin Impact character!')
                } catch (err) {
                    console.error(err)
                    await yakeys.reply(from, 'Error or character not found!', id)
                }
            break
            case prefix+'jadwaltv': // Chika chantexxzz
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                if (ar.length !== 1) return await yakeys.reply(from, ind.wrongFormat(), id)
                await yakeys.reply(from, ind.wait(), id)
                try {
                    ...
                    if (jtv.data.status === 'true') {
                        ...
                        await yakeys.sendText(from, jtvx)
                    } else {
                        await yakeys.sendText(from, 'Channel not found!')
                    }
                } catch (err) {
                    console.error(err)
                    await yakeys.reply(from, 'Error!', id)
            ...
            case prefix+'igstory':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                if (!q) return yakeys.reply(from, ind.wrongFormat(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await yakeys.reply(from, ind.wait(), id)
                misc.its(q)
                    .then(async ({ result }) => {
                        for (let i = 0; i < result.story.itemlist.length; i++) {
                            ...
                            await yakeys.sendFileFromUrl(from, urlDownload, '', 'By: VideFrelan', id)
            ...
            case prefix+'kbbi':
                ...
                misc.kbbi(q)
                    .then(async ({ result }) => {
                        await yakeys.reply(from, result.hasil, id)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await yakeys.reply(from, 'Error!', id)
            ...
            case prefix+'linestiker':
                ...
                misc.linesticker()
                    .then(async ({ result }) => {
                        ...
                        await yakeys.reply(from, lines, id)
                ...
                misc.jadwalSholat(q)
                    .then((data) => {
                        data.map(async ({isya, subuh, dzuhur, ashar, maghrib, terbit}) => {
                            ...
                            await yakeys.reply(from, msg, id)
            ...
            case prefix+'gempa':
                ...
                misc.bmkg()
                    .then(async ({ kedalaman, koordinat, lokasi, magnitude, map, potensi, waktu }) => {
                        ...
                        await yakeys.sendFileFromUrl(from, map, 'gempa.jpg', teksInfo, id)
                ...
                misc.igStalk(q)
                    .then(async ({ graphql }) => {
                        if (graphql === undefined) {
                            ...
                            await yakeys.sendFileFromUrl(from, profile_pic_url_hd, 'insta.jpg', text, id)
            ...
            case prefix+'gsmarena':
                ...
                try {
                    misc.gsmarena(q)
                        .then(async ({ result }) => {
                            await yakeys.sendFileFromUrl(from, result.image, `${result.title}.jpg`, ind.gsm(result), id)
            ...
            case prefix+'resep':
                ...
                try {
                    misc.resep(q)
                        .then(async ({ result }) => {
                            await yakeys.sendFileFromUrl(from, result.image, `${result.title}.jpg`, ind.receipt(result), id)
            ...
            case prefix+'findstiker':
                ...
                try {
                    misc.sticker(q)
                        .then(async ({ result }) => {
                            if (result.response !== 200) return await yakeys.reply(from, 'Not found!', id)
                            for (let i = 0; i < result.data.length; i++) {
                                await yakeys.sendStickerfromUrl(from, result.data[i], null, { author: authorWm, pack: packWm })
                            }
                            console.log('Success sending sticker!')
                        })
                } catch (err) {
                    console.error(err)
                    await yakeys.reply(from, `Error!\n\n${err}`, id)
                }
            break
            case prefix+'movie':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                ...
                misc.movie(q)
                    .then(async ({ result }) => {
                        ...
                        movies += '\n\nBy: VideFrelan'
                        await yakeys.reply(from, movies, id)
            ...
            case prefix+'cekongkir': // By: VideFrelan
                ...
                misc.ongkir(kurir, askot, tukot)
                    .then(async ({ result }) => {
                        ...
                        onkir += '\n\nBy: VideFrelan'
                        await yakeys.reply(from, onkir, id)
                ...
                misc.distance(kotaAsal, kotaTujuan)
                    .then(async ({ result }) => {
                        if (result.response !== 200) {
                            ...
                            await yakeys.reply(from, result.data, id)
                    ...
                    misc.ytSearch(q)
                        .then(async ({ result }) => {
                            for (let i = 0; i < 5; i++) {
                                ...
                                await yakeys.sendFileFromUrl(from, image, `${title}.jpg`, ind.ytResult(urlyt, title, channel, duration, views), id)
            ...
            case prefix+'tts':
                ...
                try {
                    ptt.save(`${speech}.mp3`, speech, async () => {
                        await yakeys.sendPtt(from, `${speech}.mp3`, id)
                    ...
                    fs.writeFile(fileInputPath, mediaData, (err) => {
                        ...
                        ffmpeg(fileInputPath)
                            ...
                            .on('end', async () => {
                                ...
                                await yakeys.sendFile(from, fileOutputPath, 'audio.mp3', '', id)
            ...
            case prefix+'toptt':
                ...
                if (isMedia && isAudio || isQuotedAudio) {
                    ...
                    await yakeys.sendPtt(from, `./temp/audio/${name}.mp3`, id)
                    ...
                    misc.playstore(q)
                        .then(async ({ result }) => {
                            for (let i = 0; i < 5; i++) {
                                ...
                                await yakeys.sendFileFromUrl(from, icon, `${title}.jpg`, ind.playstore(app_id, title, developer, description, price, free))
                            }
            ...
            case prefix+'math':
                ...
                if (typeof mathjs.evaluate(q) !== 'number') {
                    await yakeys.reply(from, ind.notNum(q), id)
                } else {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await yakeys.reply(from, `*── 「 MATH 」 ──*\n\n${q} = ${mathjs.evaluate(q)}`, id)
                }
            break
            case prefix+'shopee':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                    ...
                    misc.shopee(namaBarang, jumlahBarang)
                        .then(async ({ result }) => {
                            for (let i = 0; i < result.items.length; i++) {
                                ...
                                await yakeys.sendFileFromUrl(from, image_cover, `${nama}.jpg`, ind.shopee(nama, harga, terjual, shop_location, description, link_product))
                            }
            ...
            case prefix+'mutual':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                if (isGroupMsg) return await yakeys.reply(from, 'Command ini tidak bisa digunakan di dalam grup!', id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await yakeys.reply(from, 'Looking for a partner...', id)
                await yakeys.sendContact(from, register.getRegisteredRandomId(_registered))
                await yakeys.sendText(from, `Partner found: 🙉\n*${prefix}next* — find a new partner`)
            break
            case prefix+'next':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                if (isGroupMsg) return await yakeys.reply(from, 'Command ini tidak bisa digunakan di dalam grup!', id)
            ...
            case prefix+'tafsir':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                if (args.length === 0) return yakeys.reply(from, `Untuk menampilkan ayat Al-Qur'an tertentu beserta tafsir dan terjemahannya\ngunakan ${prefix}tafsir surah ayat\n\nContoh: ${prefix}tafsir Al-Mulk 10`, id)
                await yakeys.reply(from, ind.wait(), id)
                ...
                if (!isNaN(nomerSurah)) {
                    ...
                    await yakeys.reply(from, pesan, id)
                }
            break
            case prefix+'listsurah':
                ...
                misc.listSurah()
                    .then(async ({ result }) => {
                        ...
                        await yakeys.reply(from, list, id)
            ...
            case prefix+'surah':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                if (args.length !== 1) return await yakeys.reply(from, ind.wrongFormat(), id)
                await yakeys.reply(from, ind.wait(), id)
                misc.getSurah(args[0])
                    .then(async ({ result }) => {
                        await yakeys.reply(from, `${result.surah}\n\n${result.quran}`, id)
            ...
            case prefix+'asmaulhusna': // irham01
                ...
                yakeys.sendFileFromUrl(from, 'https://i2.wp.com/seruni.id/wp-content/uploads/2016/09/Allah.png?resize=696%2C696&ssl=1', 'gambar.jpg', ind.asmaulHusna(assna), id)
            break
            case prefix+'randomquran': // irham01
                ...
                await yakeys.reply(from, ind.randomQuran(ranquran), id)
                await yakeys.sendFileFromUrl(from, auquran, 'rquran.mp3', '', id)
            break
            case prefix+'motivasi':
                ...
                misc.motivasi()
                    .then(async (body) => {
                        ...
                        await yakeys.sendText(from, randomMotivasi)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await yakeys.reply(from, 'Error!', id)
            ...
            case prefix+'play':
                ...
                misc.ytPlay(q)
                    .then(async ({ result }) => {
                        if (Number(result.size.split(' MB')[0]) >= 10.0) return yakeys.sendFileFromUrl(from, result.image, `${result.title}.jpg`, `Judul: ${result.title}\nSize: *${result.size}*\n\nGagal, Maksimal video size adalah *10MB*!`, id)
                        await yakeys.sendFileFromUrl(from, result.image, `${result.title}.jpg`, ind.ytPlay(result), id)
                        const responses = await fetch(result.mp3)
                        const buffer = await responses.buffer()
                        await fs.writeFile(`./temp/ytplay_${sender.id}.mp3`, buffer)
                        await yakeys.sendFile(from, `./temp/ytplay_${sender.id}.mp3`, `ytplay_${sender.id}`, id)
            ...
            case prefix+'playv': // Alvio Adji Januar 
                ...
                if (getvid.data.status === false) {
                    await yakeys.reply(from, getvid.data.message, id)
                } else if (Number(getvid.data.result.size.split('MB')[0]) >= 10.00) {
                    await yakeys.reply(from, 'Maaf durasi musik sudah melebihi batas maksimal 10 MB!', id)
                } else {
                    await yakeys.sendFileFromUrl(from, getvid.data.result.thumb, 'thumb.jpg', `Title: ${getvid.data.result.title}\n\n───────────⚪───────────\n(っ◔◡◔)っ\n───────────⚪───────────\n➥Size: ${getvid.data.result.size}\n➥Type: Mp4\n➥Link Download: ${getvid.data.result.link}\n\n*Mohon Tunggu Bot Akan Mengirim Video!*`, id)
                    await yakeys.sendFileFromUrl(from, getvid.data.result.link, 'play.mp4' , '', id)
                }
            break
            case prefix+'whois':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                ...
                misc.email(emailTarget, subjectEmail, messageEmail)
                    .then(async ({ result }) => {
                        if (result.status === '204') {
                            await yakeys.reply(from, 'Server busy!', id)
                        } else {
                            await yakeys.reply(from, `*Success sending email*!\n➸ *Target*: ${emailTarget}\n➸ *Subject*: ${result.subjek}\n➸ *Message*: ${result.pesan}`, id)
            ...
            case prefix+'sms':
                ...
                misc.sms(nomorPenerima, pesanPengirim)
                    .then(async ({ status, pesan }) => {
                        if (status !== 'success') return await yakeys.reply(from, pesan, id)
                        await yakeys.reply(from, `Success sending SMS to: ${nomorPenerima}\nMessage: ${pesanPengirim}`, id)
            ...
            case prefix+'delsticker':
                ...
                if (_stick.includes(q)) {
                    ...
                    await yakeys.reply(from, ind.stickerDel(), id)
                } else {
                    await yakeys.reply(from, ind.stickerNotFound())
                }
            break
            ...
            case prefix+'liststiker':
                ...
                await yakeys.sendText(from, stickerList)
            break
            case prefix+'toxic':
                if (!isRegistered) return await yakeys.reply(from , ind.notRegistered(), id)
                await yakeys.reply(from, toxic(), id)
            break
            case prefix+'alkitab':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                if (!q) return await yakeys.reply(from, ind.wrongFormat(), id)
                ...
                misc.alkitab(q)
                    .then(async ({ result }) => {
                        ...
                        await yakeys.reply(from, alkitab, id)
                        console.log('Success sending Al-Kitab!')
                    })
            break
            case prefix+'reminder': // by YakkoXCode
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                if (!q.includes('|')) return await yakeys.reply(from, ind.wrongFormat(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                ...
                await yakeys.sendTextWithMentions(from, ind.reminderOn(messRemind, parsedTime, sender))
                const intervRemind = setInterval(async () => {
                    if (Date.now() >= reminder.getReminderTime(sender.id, _reminder)) {
                        await yakeys.sendTextWithMentions(from, ind.reminderAlert(reminder.getReminderMsg(sender.id, _reminder), sender))
            ...
            case prefix+'imgtourl':
                ...
                if (isMedia && isImage || isQuotedImage) {
                    ...
                    await yakeys.reply(from, linkImg, id)
                } else {
                    await yakeys.reply(from, ind.wrongFormat(), id)
                }
            break
            case prefix+'infohoax':
                ...
                misc.infoHoax()
                    .then(async ({ result }) => {
                        ...
                        await yakeys.sendFileFromUrl(from, result[0].image, 'hoax.jpg', txt, id)
                        console.log('Success sending info!')
                    })
                    .catch(async (err) => {
                        console.error(err)
            ...
            case prefix+'spamcall':
                ...
                misc.call(q)
                    .then(async ({ result }) => {
                        await yakeys.reply(from, result.logs, id)
            ...
            case prefix+'spamsms':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                if (args.length !== 2) return await yakeys.reply(from, ind.wrongFormat(), id)
                if (isNaN(Number(args[0])) && isNaN(Number(args[1]))) return await yakeys.reply(from, ind.wrongFormat(), id)
                if (Number(args[1]) > 10) return await yakeys.reply(from, 'Maximum 10 SMS.', id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await yakeys.reply(from, ind.wait(), id)
                misc.spamsms(args[0], args[1])
                    .then(async ({ status, logs, msg }) => {
                        if (status !== 200) {
                            ...
                            await yakeys.reply(from, logs, id)
            ...
            case prefix+'trans':
                ...
                translate(texto, {to: languaget}).then(res => {yakeys.reply(from, res.text, id)})
            break
            case prefix+'bass':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                if (isMedia && isAudio || isQuotedAudio || isVoice || isQuotedVoice) {
                    ...
                    fs.writeFile(fileInputPath, mediaData, (err) => {
                        ...
                        ffmpeg(fileInputPath)
                            ...
                            .on('end', async () => {
                                console.log(color('[FFmpeg]', 'green'), 'Processing finished!')
                                await yakeys.sendPtt(from, fileOutputPath, id)
            ...
            case prefix+'help':
                ...
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                if (args[0] === '1') {
                    await yakeys.sendText(from, ind.menuDownloader())
                } else if (args[0] === '2') {
                    await yakeys.sendText(from, ind.menuBot())
                } else if (args[0] === '3') {
                    await yakeys.sendText(from, ind.menuMisc())
                } else if (args[0] === '4') {
                    await yakeys.sendText(from, ind.menuSticker())
                } else if (args[0] === '5') {
                    await yakeys.sendText(from, ind.menuWeeaboo())
                } else if (args[0] === '6') {
                    await yakeys.sendText(from, ind.menuFun())
                } else if (args[0] === '7') {
                    await yakeys.sendText(from, ind.menuModeration())
                } else if (args[0] === '8') {
                    if (isGroupMsg && !isNsfw) return await yakeys.reply(from, ind.notNsfw(), id)
                    await yakeys.sendText(from, ind.menuNsfw())
                } else if (args[0] === '9') {
                    if (!isOwner) return await yakeys.reply(from, ind.ownerOnly())
                    await yakeys.sendText(from, ind.menuOwner())
                } else if (args[0] === '10') {
                    if (!isGroupMsg) return await yakeys.reply(from, ind.groupOnly(), id)
                    await yakeys.sendText(from, ind.menuLeveling())
                } else {
                    await yakeys.sendText(from, ind.menu(jumlahUser, levelMenu, xpMenu, role, pushname, reqXpMenu, isPremium ? 'YES' : 'NO'))
                }
            break
            case prefix+'rules':
            case prefix+'rule':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                await yakeys.sendText(from, ind.rules())
            break
            case prefix+'nsfw':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                if (!isGroupMsg) return await yakeys.reply(from, ind.groupOnly(), id)
                if (!isGroupAdmins) return await yakeys.reply(from, ind.adminOnly(), id)
                if (ar[0] === 'enable') {
                    if (isNsfw) return await yakeys.reply(from, ind.nsfwAlready(), id)
                    _nsfw.push(groupId)
                    fs.writeFileSync('./database/group/nsfw.json', JSON.stringify(_nsfw))
                    await yakeys.reply(from, ind.nsfwOn(), id)
                } else if (ar[0] === 'disable') {
                    _nsfw.splice(groupId, 1)
                    fs.writeFileSync('./database/group/nsfw.json', JSON.stringify(_nsfw))
                    await yakeys.reply(from, ind.nsfwOff(), id)
                } else {
                    await yakeys.reply(from, ind.wrongFormat(), id)
                }
            break
            case prefix+'status':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                await yakeys.sendText(from, `*RAM*: ${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)} MB / ${Math.round(os.totalmem / 1024 / 1024)} MB\n*CPU*: ${os.cpus()[0].model}`)
            break
            case prefix+'listblock':
                ...
                await yakeys.sendTextWithMentions(from, block)
            break
            case prefix+'ownerbot':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                await yakeys.sendContact(from, ownerNumber)
            break
            case prefix+'runtime': // BY HAFIZH
                ...
                await yakeys.reply(from, `*── 「 BOT UPTIME 」 ──*\n\n❏${formater(uptime)}`, id)
            break
            case prefix+'ping':
            case prefix+'p':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                await yakeys.sendText(from, `Pong!\nSpeed: ${processTime(t, moment())} secs`)
            break
            case prefix+'delete':
            case prefix+'del':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                if (!quotedMsg) return await yakeys.reply(from, ind.wrongFormat(), id)
                if (!quotedMsgObj.fromMe) return await yakeys.reply(from, ind.wrongFormat(), id)
                await yakeys.deleteMessage(quotedMsgObj.chatId, quotedMsgObj.id, false)
            break
            case prefix+'report':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                if (!q) return await yakeys.reply(from, ind.emptyMess(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                ...
                if (lastReport !== undefined && cd - (Date.now() - lastReport) > 0) {
                    ...
                    await yakeys.reply(from, ind.daily(time), id)
                } else {
                    if (isGroupMsg) {
                        await yakeys.sendText(ownerNumber, `*── 「 REPORT 」 ──*\n\n*From*: ${pushname}\n*ID*: ${sender.id}\n*Group*: ${(name || formattedTitle)}\n*Message*: ${q}`)
                        await yakeys.reply(from, ind.received(pushname), id)
                    } else {
                        await yakeys.sendText(ownerNumber, `*── 「 REPORT 」 ──*\n\n*From*: ${pushname}\n*ID*: ${sender.id}\n*Message*: ${q}`)
                        await yakeys.reply(from, ind.received(pushname), id)
                    }
            ...
            case prefix+'tos':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                await yakeys.sendLinkWithAutoPreview(from, 'https://github.com/YakkoXCode/YakkoXCode', ind.tos(ownerNumber))
            break
            case prefix+'join':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                if (!isUrl(url) && !url.includes('chat.whatsapp.com')) return await yakeys.reply(from, ind.wrongFormat(), id)
                const checkInvite = await yakeys.inviteInfo(url)
                if (isOwner) {
                    await yakeys.joinGroupViaLink(url)
                    await yakeys.reply(from, ind.ok(), id)
                    await yakeys.sendText(checkInvite.id, `Hello!! I was invited by ${pushname}`)
                } else {
                    const getGroupData = await yakeys.getAllGroups()
                    if (getGroupData.length >= groupLimit) {
                        await yakeys.reply(from, `Invite refused. Max group is: ${groupLimit}`, id)
                    } else if (getGroupData.size <= memberLimit) {
                        await yakeys.reply(from, `Invite refused. Minimum member is: ${memberLimit}`, id)
                    } else {
                        if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                        limit.addLimit(sender.id, _limit, isPremium, isOwner)
                        await yakeys.joinGroupViaLink(url)
            ...
            case prefix+'cekpremium':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                if (!isPremium) return await yakeys.reply(from, ind.notPremium(), id)
                const cekExp = ms(premium.getPremiumExpired(sender.id, _premium) - Date.now())
                await yakeys.reply(from, `*── 「 PREMIUM EXPIRED 」 ──*\n\n➸ *ID*: ${sender.id}\n➸ *Premium left*: ${cekExp.days} day(s) ${cekExp.hours} hour(s) ${cekExp.minutes} minute(s)`, id)
            break
            ...
            case prefix+'listpremium':
                ...
                for (let i = 0; i < deret.length; i++) {
                    ...
                    arrayPremi.push(await yakeys.getContact(premium.getAllPremiumUser(_premium)[i]))
                    listPremi += `${i + 1}. wa.me/${premium.getAllPremiumUser(_premium)[i].replace('@c.us', '')}\n➸ *Name*: ${arrayPremi[i].pushname}\n➸ *Expired*: ${checkExp.days} day(s) ${checkExp.hours} hour(s) ${checkExp.minutes} minute(s)\n\n`
                }
                await yakeys.reply(from, listPremi, id)
            break
            case prefix+'getpic':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                if (mentionedJidList.length !== 0) {
                    const userPic = await yakeys.getProfilePicFromServer(mentionedJidList[0])
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    ...
                    await yakeys.sendFileFromUrl(from, pek, 'pic.jpg', '', id)
                } else if (args.length !== 0) {
                    const userPic = await yakeys.getProfilePicFromServer(args[0] + '@c.us')
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    if (userPic === undefined) {
                        var peks = errorImg
                    ...
                    await yakeys.sendFileFromUrl(from, peks, 'pic.jpg', '', id)
                } else {
                    await yakeys.reply(from, ind.wrongFormat(), id)
                }
            break
            case prefix+'serial':
                if (!isRegistered) return await yakeys.reply(from, ind.registered(), id)
                if (isGroupMsg) return await yakeys.reply(from, ind.pcOnly(), id)
                if (args.length !== 1) return await yakeys.reply(from, ind.wrongFormat(), id)
                ...
                if (register.checkRegisteredUserFromSerial(serials, _registered)) {
                    ...
                    await yakeys.sendText(from, ind.registeredFound(name, age, time, serials, id))
                } else {
                    await yakeys.sendText(from, ind.registeredNotFound(serials))
                }
            break
            case prefix+'limit':
                if (isPremium || isOwner) return await yakeys.reply(from, '⤞ Limit left: ∞ (UNLIMITED)', id)
                await yakeys.reply(from, `⤞ Limit left: ${limit.getLimit(sender.id, _limit, limitCount)} / 25\n\n*_Limit direset pada pukul 00:00 WIB_*`, id)
            break
            ...
            case prefix+'neko':
                ...
                await yakeys.sendFileFromUrl(from, (await neko.sfw.neko()).url, 'neko.jpg', '', null, null, true)
            ...
            case prefix+'chara':
                ...
                try {
                    ...
                    await yakeys.sendFileFromUrl(from, image.large, `${q}.jpg`, `${text_1}`, id)
                } catch (err) {
                    console.error(err)
                    await yakeys.reply(from, 'Error or chara not foud!', id)
                }
            break
            ...
            case prefix+'wp':
                ...
                await yakeys.sendFileFromUrl(from, (await neko.sfw.wallpaper()).url, 'wallpaper.jpg', '', null, null, true)
            ...
            case prefix+'kemono':
                ...
                await yakeys.sendFileFromUrl(from, (await neko.sfw.kemonomimi()).url, 'kemono.jpg', '', null, null, true)
            ...
            case prefix+'kusonime':
                ...
                weeaboo.anime(q)
                    .then(async ({ info, link_dl, sinopsis, thumb, title, error, status }) => {
                        if (status === false) {
                            return await yakeys.reply(from, error, id)
                        } else {
                            let animek = `${title}\n\n${info}\n\nSinopsis: ${sinopsis}\n\nLink download:\n${link_dl}`
                            await yakeys.sendFileFromUrl(from, thumb, 'animek.jpg', animek, null, null, true)
            ...
            case prefix+'komiku':
                ...
                weeaboo.manga(q)
                    .then(async ({ genre, info, link_dl, sinopsis, thumb }) => {
                        ...
                        await yakeys.sendFileFromUrl(from, thumb, 'mangak.jpg', mangak, null, null, true)
                    ...
                    weeaboo.wait(imageBase64)
                        .then(async (result) => {
                            if (result.docs && result.docs.length <= 0) {
                                return await yakeys.reply(from, 'Anime not found! :(', id)
                            } else {
                                ...
                                if (similarity < 0.92) {
                                    ...
                                    await yakeys.sendFileFromUrl(from, video, `${title_romaji}.mp4`, teks, id)
                    ...
                    try {
                        ...
                        for (let i = 0; i < results.length; i++) {
                            ...
                            if (results[i].similarity < 80.00) {
                                ...
                                await yakeys.sendLinkWithAutoPreview(from, results[i].url, teks)
            ...
            case prefix+'waifu':
                ...
                weeaboo.waifu(false)
                    .then(async ({ url }) => {
                        await yakeys.sendFileFromUrl(from, url, 'waifu.png', '', id)
            ...
            case prefix+'anitoki':
                ...
                weeaboo.anitoki()
                    .then(async ({ result }) => {
                        ...
                        await yakeys.reply(from, anitoki, id)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await yakeys.reply(from, 'Error!', id)
            ...
            case prefix+'neonime':
                ...
                weeaboo.neonime()
                    .then(async ({ status, result }) => {
                        if (status !== 200) return await yakeys.reply(from, 'Not found.', id)
                        let neoInfo = '*── 「 NEONIME LATEST 」 ──*'
                        for (let i = 0; i < result.length; i++) {
                            const { date, title, link, desc } = result[i]
                            neoInfo += `\n\n➸ *Title*: ${title}\n➸ *Date*: ${date}\n➸ *Synopsis*: ${desc}\n➸ *Link*: ${link}\n\n=_=_=_=_=_=_=_=_=_=_=_=_=`
                        }
                        await yakeys.reply(from, neoInfo, id)
            ...
            case prefix+'anoboy':
                ...
                weeaboo.anoboy()
                    .then(async ({ result }) => {
                        ...
                        await yakeys.reply(from, anoboyInfo, id)
            ...
            case prefix+'animesticker': 
                ...
                weeaboo.snime()
                    .then(async (body) => {
                        ...
                        await yakeys.sendStickerfromUrl(from, wifegerakx)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await yakeys.reply(from, 'Error!', id)
            ...
            case prefix+'bapak': // By Kris
                ...
                axios.get(`https://api.terhambar.com/bpk?kata=${q}`)
                    .then(async (res) => await yakeys.reply(from, res.data.text, id))
                    .catch(async (err) => {
                        console.error(err)
                        await yakeys.reply(from, 'Error!', id)
                    })
            ...
            case prefix+'puisi': // By Kris
                ...
                axios.get('https://masgi.herokuapp.com/api/puisi2')
                    .then(async (res) => await yakeys.reply(from, res.data.data, id))
                    .catch(async (err) => {
                        console.error(err)
                        await yakeys.reply(from, 'Error!', id)
                    })
            ...
            case prefix+'creepyfact': // By Kris
                ...
                fetch('https://raw.githubusercontent.com/TheSploit/CreepyFact/main/creepy.txt')
                    ...
                    .then(async (body) => {
                        ...
                        await yakeys.reply(from, creepyz, id)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await yakeys.reply(from, 'Error!', id)
            ...
            case prefix+'quotes':
                ...
                misc.quotes()
                .then(async ({ result }) => {
                    await yakeys.reply(from, `➸ *Quotes*: ${result.quotes}\n➸ *Author*: ${result.author}`, id)
                })
            break
            case prefix+'asupan': // shansekai
                ...
                fun.asupan()
                    .then(async (body) => {
                        ...
                        await yakeys.sendFileFromUrl(from, `http://sansekai.my.id/ptl_repost/${asupanx}`, 'asupan.mp4', 'Follow IG: https://www.instagram.com/ptl_repost untuk mendapatkan asupan lebih banyak.', id)
            ...
            case prefix+'citacita': // Piyobot
                ...
                fun.cita()
                    .then(async (body) => {
                        ...
                        await yakeys.sendFileFromUrl(from, randomCita, 'cita.mp3', '', id)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await yakeys.reply(from, 'Error!', id)
            ...
            case prefix+'dadu': // by CHIKAA CHANTEKKXXZZ
                ...
                fun.dadu()
                    .then(async (body) => {
                        ...
                        await yakeys.sendStickerfromUrl(from, dadugerakx, null, { author: authorWm, pack: packWm })
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await yakeys.reply(from, 'Error!', id)
            ...
            case prefix+'doge':
                ...
                fun.doge()
                    .then(async (body) => {
                        ...
                        await yakeys.sendStickerfromUrl(from, dogegx, null, { author: authorWm, pack: packWm })
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await yakeys.reply(from, 'Error!', id)
            ...
            case prefix+'me':
                ...
                if (quotedMsg) {
                    ...
                    const profilePic = await yakeys.getProfilePicFromServer(getQuoted)
                    const username = quotedMsgObj.sender.name
                    const statuses = await yakeys.getStatus(getQuoted)
                    ...
                    await yakeys.sendFileFromUrl(from, pfp, `${username}.jpg`, ind.profile(username, status, premi, benet, adm, levelMe, req, xpMe), id)
                } else {
                    const profilePic = await yakeys.getProfilePicFromServer(sender.id)
                    const username = pushname
                    const statuses = await yakeys.getStatus(sender.id)
                    ...
                    await yakeys.sendFileFromUrl(from, pfps, `${username}.jpg`, ind.profile(username, status, premi, benet, adm, levelMe, req, xpMe), id)
                }
            break
            case prefix+'hartatahta':
                ...
                await yakeys.sendFileFromUrl(from, `https://api.vhtear.com/hartatahta?text=${q}&apikey=${config.vhtear}`, `${q}.jpg`, '', id)
            ...
            case prefix+'nulis':
                ...
                await yakeys.sendFileFromUrl(from, `https://api.vhtear.com/write?text=${q}&apikey=${config.vhtear}`, 'nulis.jpg', '', id)
            ...
            case prefix+'ffbanner': // By: VideFrelan
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                if (!q.includes('|')) return yakeys.reply(from, ind.wrongFormat(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await yakeys.reply(from, ind.wait(), id)
                ...
                await yakeys.sendFileFromUrl(from, `https://api.vhtear.com/bannerff?title=${teks1ffanjg}&text=${teks2ffanjg}&apikey=${config.vhtear}`, id)
                console.log('Success!')
            break
            case prefix+'caklontong': //By: VideFrelan
                if (!isGroupMsg) return yakeys.reply(from, ind.groupOnly(), id)
                if (!isRegistered) return  yakeys.reply(from, ind.notRegistered(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await yakeys.reply(from, ind.wait(), id)
                const sleep = (ms) => {
                ...
                fun.caklontong()
                    .then(async ( { result }) => {
                        await yakeys.reply(from, `➸ *Soal*: ${result.soal}`, id)
                        await yakeys.sendText(from, '30 Detik Tersisa...')
                        await sleep(10000)
                        await yakeys.sendText(from, '20 Detik Tersisa...')
                        await sleep(10000)
                        await yakeys.sendText(from, '10 Detik Tersisa...')
                        await sleep(10000)
                        await yakeys.reply(from, `➸ *Jawaban*: ${result.jawaban}\n${result.desk}`, id)
            ...
            case prefix+'tebakgambar':
                ...
                fun.tbkgmbr()
                    .then(async ({ result }) => {
                        await yakeys.sendFileFromUrl(from, result.soal_gbr, 'TebakGambar.jpg', '', id)
                        await yakeys.sendText(from, '50 Detik Tersisa...')
                        await tsleep(10000)
                        await yakeys.sendText(from, '40 Detik Tersisa...')
                        await tsleep(10000)
                        await yakeys.sendText(from, '30 Detik Tersisa...')
                        await tsleep(10000)
                        await yakeys.sendText(from, '20 Detik Tersisa...')
                        ...
                        await yakeys.sendText(from, '10 Detik Tersisa...')
                        await tsleep(10000)
                        await yakeys.reply(from, `➸ *Jawaban*: ${result.jawaban}`, id)
            ...
            case prefix+'fflogo': // By: VideFrelan
                ...
                await yakeys.sendFileFromUrl(from, `https://api.vhtear.com/logoff?hero=${karakter}&text=${teksff}&apikey=${config.vhtear}`, id)
                console.log('Success!')
            break
            case prefix+'text3d':
            case prefix+'3dtext':
                ...
                await yakeys.sendFileFromUrl(from, `https://docs-jojo.herokuapp.com/api/text3d?text=${q}`,`${q}.jpg`, '', id)
            ...
            case prefix+'simi': // by: VideFrelan
                ...
                fun.simi(q)
                    .then(async ({ jawaban }) => {
                        await yakeys.reply(from, jawaban, id)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await yakeys.reply(from, `Error!\n\n${err}`, id)
            ...
            case prefix+'glitext':
                ...
                await yakeys.sendFileFromUrl(from, `https://api.vhtear.com/glitchtext?text1=${teks1}&text2=${teks2}&apikey=${config.vhtear}`, 'glitch.jpg', '', id)
                    .then(() => console.log('Success creating image!'))
                    .catch(async (err) => {
                        console.error(err)
                        await yakeys.reply(from, 'Error!', id)
            ...
            case prefix+'phmaker':
                ...
                await yakeys.sendFileFromUrl(from, `https://api.vhtear.com/pornlogo?text1=${kiri}&text2=${kanan}&apikey=${config.vhtear}`, 'ph.jpg', '', id)
                    .then(() => console.log('Success creating image!'))
                    .catch(async (err) => {
                        console.error(err)
                        await yakeys.reply(from, 'Error!', id)
            ...
            case prefix+'blackpink':
                ...
                await yakeys.sendFileFromUrl(from, `https://api.vhtear.com/blackpinkicon?text=${q}&apikey=${config.vhtear}`, `${q}.jpg`, '', id)
            ...
            case prefix+'galaxy':
                ...
                await yakeys.sendFileFromUrl(from, `https://api.vhtear.com/galaxytext?text=${q}&apikey=${config.vhtear}`, `${q}.jpg`, '', id)
                    .then(() => console.log('Success creating image!'))
                    .catch(async (err) => {
                        console.error(err)
                        await yakeys.reply(from, 'Error!', id)
            ...
            case prefix+'tod':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                await yakeys.reply(from, 'Sebelum bermain berjanjilah akan melaksanakan apapun perintah yang diberikan.' , id)
                await yakeys.sendText(from, `Silakan ketik *${prefix}truth* atau *${prefix}dare*`)
            break
            case prefix+'weton':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                if (!q.includes('|')) return await yakeys.reply(from, ind.wrongFormat(), id)
            ...
            case prefix+'truth':
                ...
                fun.truth()
                    .then(async (body) => {
                        ...
                        await yakeys.reply(from, randomTod, id)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await yakeys.reply(from, 'Error!', id)
            ...
            case prefix+'hilih':
                ...
                fun.hilihteks(q)
                    .then(async ( { result }) => {
                        await yakeys.reply(from, result.kata, id)
            ...
            case prefix+'dare':
                ...
                fun.dare()
                    .then(async (body) => {
                        ...
                        await yakeys.reply(from, randomDare, id)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await yakeys.reply(from, 'Error!', id)
                    ...
                    canvas.Canvas.trigger(mediaData)
                        .then((buffer) => {
                            ...
                            ffmpeg(fileInputPath)
                                ...
                                .on('end', async () => {
                                    console.log(color('[FFmpeg]', 'green'), 'Processing finished!')
                                    await yakeys.sendMp4AsSticker(from, fileOutputPath, { fps: 30, startTime: '00:00:00.0', endTime : '00:00:05.0', loop: 0 })
            ...
            case prefix+'trash':
                ...
                try {
                    ...
                    for (let i = 0; i < mentionedJidList.length; i++) {
                        const ypics = await yakeys.getProfilePicFromServer(mentionedJidList[i])
                        if (ypics === undefined) {
                            var ypfps = errorImg
                        } else {
                            ypfps = ypics
                    ...
                    canvas.Canvas.trash(ypfps)
                        .then(async (buffer) => {
                            ...
                            await yakeys.sendFile(from, `./temp/${sender.id}_trash.png`, `${sender.id}_trash.png`, '', id)
                ...
                try {
                    ...
                    canvas.Canvas.hitler(ypf)
                        .then(async (buffer) => {
                            ...
                            await yakeys.sendFile(from, `./temp/${sender.id}_hitler.png`, `${sender.id}_hitler.png`, '', id)
            ...
            case prefix+'wasted':
                ...
                if (isMedia && type === 'image' || isQuotedImage) {
                    ...
                    await yakeys.reply(from, ind.wait(), id)
                    await yakeys.sendFileFromUrl(from, `https://some-random-api.ml/canvas/wasted?avatar=${fotoWtNya}`, 'Wasted.jpg', 'Ini..., sticker nya lagi di kirim', id).then(() => yakeys.sendStickerfromUrl(from, `https://some-random-api.ml/canvas/wasted?avatar=${fotoWtNya}`))
            ...
            case prefix+'kiss':
                ...
                try {
                    if (isMedia && isImage || isQuotedImage) {
                        ...
                        await yakeys.reply(from, ind.wait(), id)
                        const encryptMedia = isQuotedImage ? quotedMsg : message
                        const ppRaw = await yakeys.getProfilePicFromServer(sender.id)
                        ...
                        canvas.Canvas.kiss(ppFirst, ppSecond)
                            .then(async (buffer) => {
                                ...
                                await yakeys.sendFile(from, `${sender.id}_kiss.png`, `${sender.id}_kiss.png`, '', id)
            ...
            case prefix+'phcomment':
                ...
                const ppPhRaw = await yakeys.getProfilePicFromServer(sender.id)
                ...
                await yakeys.sendFileFromUrl(from, preprocessPh.data.message, 'ph.jpg', '', id)
            ...
            case prefix+'neon':
                ...
                await yakeys.reply(from, ind.wait(), id)
                await yakeys.sendFileFromUrl(from, `http://docs-jojo.herokuapp.com/api/neon?text1=${atasnya}&text2=${tengahnya}&text3=${bawahnya}`, 'neon.jpg', '', id)
                console.log('Success creating image!')
            break
            case prefix+'firemaker':
                ...
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await yakeys.reply(from, ind.wait(), id)
                await yakeys.sendFileFromUrl(from, `https://api.vhtear.com/fire_maker?text=${q}&apikey=${config.vhtear}`)
                console.log('Success creating image!')
            break
            case prefix+'mlmaker':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                ...
                await yakeys.reply(from, ind.wait(), id)
                await yakeys.sendFileFromUrl(from, `https://api.vhtear.com/logoml?hero=${namaHero}&text=${teksMl}&apikey=${config.vhtear}`)
                console.log('Success creating image!')
            break
            ...
            case prefix+'blmaker':
                ...
                await yakeys.reply(from, ind.wait(), id)
                await yakeys.sendFileFromUrl(from, `https://api.vhtear.com/balloonmaker?text1=${namaKiri}&text2=${namaKanan}&apikey=${config.vhtear}`)
                console.log('Success creating image!')
            break
            case prefix+'sliding':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                ...
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await yakeys.reply(from, ind.wait(), id)
                await yakeys.sendVideoAsGif(from, `https://api.vhtear.com/slidingtext?text=${q}&apikey=${config.vhtear}`, 'sliding.gif', '', id)
            ...
            case prefix+'stickernobg': // by: VideFrelan
                ...
                if (isMedia && isImage || isQuotedImage) {
                    ...
                    misc.stickernobg(q)
                        .then(async ({ result }) => {
                            await yakeys.sendStickerfromUrl(from, result.image, null, { author: authorWm, pack: packWm })
                            console.log(`Sticker processed for ${processTime(t, moment())} seconds`)
                        })
                        .catch(async (err) => {
                            console.error(err)
            ...
            break
            case prefix+'stickerwm': // By YakkoXCode
            case prefix+'stcwm':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                if (!isPremium) return await yakeys.reply(from, ind.notPremium(), id)
                if (!q.includes('|')) return await yakeys.reply(from, ind.wrongFormat(), id)
                if (isMedia && isImage || isQuotedImage) {
                    ...
                    const _mimetype = isQuotedImage ? quotedMsg.mimetype : mimetype
                    const imageBase64 = `data:${_mimetype};base64,${mediaData.toString('base64')}`
                    await yakeys.sendImageAsSticker(from, imageBase64, { author: author, pack: packname })
                    console.log(`Sticker processed for ${processTime(t, moment())} seconds`)
                } else {
                    await yakeys.reply(from, ind.wrongFormat(), id)
                }
            ...
            case prefix+'stcmeme':
                ...
                if (isMedia && isImage || isQuotedImage) {
                    ...
                    await yakeys.sendStickerfromUrl(from, create, null, { author: authorWm, pack: packWm, keepScale: true })
                } else {
                    await yakeys.reply(from, ind.wrongFormat(), id)
                }
            break
            ...
            case prefix+'stikerp':
                ...
                if (isMedia && isImage || isQuotedImage) {
                    ...
                    const _mimetype = isQuotedImage ? quotedMsg.mimetype : mimetype
                    const imageBase64 = `data:${_mimetype};base64,${mediaData.toString('base64')}`
                    await yakeys.sendImageAsSticker(from, imageBase64, { author: authorWm, pack: packWm, keepScale: true })
                    console.log(`Sticker processed for ${processTime(t, moment())} seconds`)
                } else {
                    await yakeys.reply(from, ind.wrongFormat(), id)
                }
            ...
            case prefix+'sgif':
                ...
                if (isMedia && isVideo || isGif || isQuotedVideo || isQuotedGif) {
                    ...
                    try {
                        ...
                        const mediaData = await decryptMedia(encryptMedia, uaOverride)
                        const videoBase64 = `data:${mimetype};base64,${mediaData.toString('base64')}`
                        await yakeys.sendMp4AsSticker(from, videoBase64, null, { stickerMetadata: true, author: authorWm, pack: packWm, fps: 30, startTime: '00:00:00.0', endTime : '00:00:05.0', crop: false, loop: 0 })
                            .then(() => {
                                console.log(`Sticker processed for ${processTime(t, moment())} seconds`)
                            })
                    } catch (err) {
            ...
            case prefix+'ttg':
                ...
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await yakeys.reply(from, ind.wait(), id)
                await yakeys.sendStickerfromUrl(from, `https://api.vhtear.com/textxgif?text=${q}&apikey=${config.vhtear}`, null, { author: authorWm, pack: packWm })
            ...
            case prefix+'toimg':
                ...
                if (isQuotedSticker) {
                    ...
                    try {
                        ...
                        await yakeys.sendFile(from, imageBase64, 'sticker.jpg', '', id)
                    } catch (err) {
                        console.error(err)
                        await yakeys.reply(from, 'Error!', id)
                    }
            ...
            case prefix+'emojistiker':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                if (args.length !== 1) return yakeys.reply(from, ind.wrongFormat(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                const emoji = emojiUnicode(args[0])
                await yakeys.reply(from, ind.wait(), id)
                console.log('Creating emoji code for =>', emoji)
                await yakeys.sendStickerfromUrl(from, `https://api.vhtear.com/emojitopng?code=${emoji}&apikey=${config.vhtear}`, null, { author: authorWm, pack: packWm })
                    .then(async () => {
                        await yakeys.reply(from, ind.ok(), id)
                        console.log(`Sticker processed for ${processTime(t, moment())} seconds`)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await yakeys.reply(from, 'Emoji not supported!', id)
                    })
            break
            ...
            case prefix+'mlewd':
                ...
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                if (isGroupMsg) {
                    if (!isNsfw) return await yakeys.reply(from, ind.notNsfw(), id)
                    if (!isPremium) return await yakeys.reply(from, ind.notPremium(), id)
                    await yakeys.reply(from, ind.botNotPremium(), id)
                } else {
                    if (!isPremium) return await yakeys.reply(from, ind.notPremium(), id)
                    await yakeys.reply(from, ind.botNotPremium(), id)
                }
            ...
            case prefix+'lewd':
                ...
                if (isGroupMsg) {
                    ...
                    nsfw.randomLewd()
                        .then(async ({ url }) => {
                            await yakeys.sendFileFromUrl(from, url, 'lewd.jpg', '', null, null, true)
                ...
                if (isGroupMsg) {
                    ...
                    try {
                        if (ar[0] === 'armpits') {
                            nsfw.armpits()
                                .then(async ({ url }) => {
                                    await yakeys.sendFileFromUrl(from, url, 'armpits.jpg', '', id)
                                        .then(() => console.log('Success sending armpits pic!'))
                                })
                        } else if (ar[0] === 'feets') {
                            nsfw.feets()
                                .then(async ({ url }) => {
                                    await yakeys.sendFileFromUrl(from, url, 'feets.jpg', '', id)
                                        .then(() => console.log('Success sending feets pic!'))
                                })
                        } else if (ar[0] === 'thighs') {
                            nsfw.thighs()
                                .then(async ({ url }) => {
                                    await yakeys.sendFileFromUrl(from, url, 'thighs.jpg', '', id)
                                        .then(() => console.log('Success sending thighs pic!'))
                                })
                        } else if (ar[0] === 'ass') {
                            nsfw.ass()
                                .then(async ({ url }) => {
                                    await yakeys.sendFileFromUrl(from, url, 'ass.jpg', '', id)
                                        .then(() => console.log('Success sending ass pic!'))
                                })
                        } else if (ar[0] === 'boobs') {
                            nsfw.boobs()
                                .then(async ({ url }) => {
                                    await yakeys.sendFileFromUrl(from, url, 'boobs.jpg', '', id)
                                        .then(() => console.log('Success sending boobs pic!'))
                                })
                        } else if (ar[0] === 'belly') {
                            nsfw.belly()
                                .then(async ({ url }) => {
                                    await yakeys.sendFileFromUrl(from, url, 'belly.jpg', '', id)
                                        .then(() => console.log('Success sending belly pic!'))
                                })
                        } else if (ar[0] === 'sideboobs') {
                            nsfw.sideboobs()
                                .then(async ({ url }) => {
                                    await yakeys.sendFileFromUrl(from, url, 'sideboobs.jpg', '', id)
                                        .then(() => console.log('Success sending sideboobs pic!'))
                                })
                        } else if (ar[0] === 'ahegao') {
                            nsfw.ahegao()
                                .then(async ({ url }) => {
                                    await yakeys.sendFileFromUrl(from, url, 'ahegao.jpg', '', id)
                                        .then(() => console.log('Success sending ahegao pic!'))
                                })
                        } else {
                            await yakeys.reply(from, 'Tag not found.', id)
                        }
                    } catch (err) {
                        console.error(err)
                        await yakeys.reply(from, err, id)
                    }
                } else {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
            ...
            case prefix+'nh':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                if (args.length !== 1) return await yakeys.reply(from, ind.wrongFormat(), id)
                if (isNaN(Number(args[0]))) return await yakeys.reply(from, ind.wrongFormat(), id)
                if (isGroupMsg) {
                    if (!isNsfw) return await yakeys.reply(from, ind.notNsfw(), id)
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    ...
                    if (validate === true) {
                        try {
                            ...
                            await yakeys.sendFileFromUrl(from, pic, 'nhentai.jpg', teks, id)
                        ...
                        await yakeys.reply(from, ind.nhFalse(), id)
                    }
                } else {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                ...
                if (isGroupMsg) {
                    ...
                    nana.search(q)
                        .then(async (g) => {
                            ...
                            await yakeys.sendFileFromUrl(from, g.results[0].thumbnail.s, `${g.results[0].title}`, txt, id)
            ...
            case prefix+'nekopoi':
                ...
                if (isGroupMsg) {
                    ...
                    try {
                        ...
                        await yakeys.reply(from, text, id)
                    } catch (err) {
                        console.error(err)
                        await yakeys.reply(from, 'Error!', id)
                    }
            ...
            case prefix+'lolivid':  //Piyobot
                ...
                weeaboo.loli()
                    .then(async (body) => {
                        ...
                        await yakeys.sendFileFromUrl(from, papololi, 'loli.mp4', '', id)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await yakeys.reply(from, 'Error!', id)
            ...
            case prefix+'phdl':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                if (!isUrl(url) && !url.includes('pornhub.com')) return await yakeys.reply(from, ind.wrongFormat(), id)
                if (isGroupMsg) {
                    if (!isNsfw) return await yakeys.reply(from, ind.notNsfw(), id)
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                        ...
                        nsfw.phDl(url)
                            .then(async ({ title, download_urls, thumbnail_url }) => {
                                ...
                                if (count !== 2) {
                                    ...
                                    await yakeys.sendFileFromUrl(from, thumbnail_url, `${title}`, `Title: ${title}\n\nLinks:\n${shortsLow} (240P)\n${shortsMid} (480P)\n${shortsHigh} (720P)`, id)
            ...
            case prefix+'yuri':
                ...
                if (isGroupMsg) {
                    ...
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await yakeys.reply(from, ind.wait(), id)
                    await yakeys.sendFileFromUrl(from, (await neko.nsfw.eroYuri()).url, 'yuri.jpg', '', id)
                } else {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await yakeys.reply(from, ind.wait(), id)
            ...
            case prefix+'lewdavatar':
                ...
                if (isGroupMsg) {
                    ...
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await yakeys.reply(from, ind.wait(), id)
                    await yakeys.sendFileFromUrl(from, (await neko.nsfw.avatar()).url, 'avatar.jpg', '', id)
                } else {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await yakeys.reply(from, ind.wait(), id)
            ...
            case prefix+'femdom':
                ...
                if (isGroupMsg) {
                    ...
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await yakeys.reply(from, ind.wait(), id)
                    await yakeys.sendFileFromUrl(from, (await neko.nsfw.femdom()).url, 'femdom.jpg', '', id)
                } else {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await yakeys.reply(from, ind.wait(), id)
            ...
            case prefix+'cersex':
                ...
                if (isGroupMsg) {
                    ...
                    nsfw.cersex()
                        .then(async ({ result }) => {
                            await yakeys.sendFileFromUrl(from, result.image, 'cersex.jpg', `*── 「 ${result.judul} 」 ──*\n\n${result.cerita}`, id)
            ...
            case prefix+'revoke':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                if (!isGroupMsg) return await yakeys.reply(from, ind.groupOnly(), id)
                if (!isGroupAdmins) return yakeys.reply(from, ind.adminOnly(), id)
                if (!isBotGroupAdmins) return yakeys.reply(from, ind.botNotAdmin(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await yakeys.revokeGroupInviteLink(groupId)
                await yakeys.sendTextWithMentions(from, `Group link revoked by @${sender.id.replace('@c.us', '')}`)
            break
            case prefix+'grouplink':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                if (!isGroupMsg) return await yakeys.reply(from, ind.groupOnly(), id)
                if (!isGroupAdmins) return await yakeys.reply(from, ind.adminOnly(), id)
                if (!isBotGroupAdmins) return await yakeys.reply(from, ind.botNotAdmin(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                const gcLink = await yakeys.getGroupInviteLink(groupId)
                await yakeys.reply(from, gcLink, id)
            break
            case prefix+'mutegc':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                if (!isGroupMsg) return yakeys.reply(from, ind.groupOnly(), id)
                ...
                if (ar[0] === 'enable') {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await yakeys.setGroupToAdminsOnly(groupId, true)
                    await yakeys.sendText(from, ind.gcMute())
                } else if (ar[0] === 'disable') {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await yakeys.setGroupToAdminsOnly(groupId, false)
                    await yakeys.sendText(from, ind.gcUnmute())
                } else {
                    await yakeys.reply(from, ind.wrongFormat(), id)
                }
            break
            case prefix+'add':
                ...
                try {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await yakeys.addParticipant(from, `${args[0]}@c.us`)
                    await yakeys.sendText(from, '🎉 Welcome! 🎉')
                } catch (err) {
                    console.error(err)
                    await yakeys.reply(from, 'Error!', id)
                }
            ...
            case prefix+'kick':
                ...
                if (!isGroupAdmins) return await yakeys.reply(from, ind.adminOnly(), id)
                if (!isBotGroupAdmins) return await yakeys.reply(from, ind.botNotAdmin(), id)
                if (mentionedJidList.length === 0) return await yakeys.reply(from, ind.wrongFormat(), id)
                if (mentionedJidList[0] === botNumber) return await yakeys.reply(from, ind.wrongFormat(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await yakeys.sendTextWithMentions(from, `Good bye~\n${mentionedJidList.map(x => `@${x.replace('@c.us', '')}`).join('\n')}`)
                for (let i of mentionedJidList) {
                    if (groupAdmins.includes(i)) return await yakeys.sendText(from, ind.wrongFormat())
                    await yakeys.removeParticipant(groupId, i)
                }
            break
            case prefix+'promote':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                ...
                if (!isGroupAdmins) return await yakeys.reply(from, ind.adminOnly(), id)
                if (!isBotGroupAdmins) return await yakeys.reply(from, ind.botNotAdmin(), id)
                if (mentionedJidList.length !== 1) return await yakeys.reply(from, ind.wrongFormat(), id)
                if (mentionedJidList[0] === botNumber) return await yakeys.reply(from, ind.wrongFormat(), id)
                if (groupAdmins.includes(mentionedJidList[0])) return await yakeys.reply(from, ind.adminAlready(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await yakeys.promoteParticipant(groupId, mentionedJidList[0])
                await yakeys.reply(from, ind.ok(), id)
            break
            case prefix+'demote':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                ...
                if (mentionedJidList.length !== 1) return await yakeys.reply(from, ind.wrongFormat(), id)
                if (mentionedJidList[0] === botNumber) return await yakeys.reply(from, ind.wrongFormat(), id)
                if (!groupAdmins.includes(mentionedJidList[0])) return await yakeys.reply(from, ind.notAdmin(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await yakeys.demoteParticipant(groupId, mentionedJidList[0])
                await yakeys.reply(from, ind.ok(), id)
            break
            case prefix+'leave':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                if (!isGroupMsg) return await yakeys.reply(from, ind.groupOnly(), id)
                if (!isGroupAdmins) return await yakeys.reply(from, ind.adminOnly(), id)
                await yakeys.sendText(from, 'Bye~ 👋')
                await yakeys.leaveGroup(groupId)
            break
            case prefix+'admins':
            case prefix+'admin':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                ...
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                const groupAdm = await yakeys.getGroupAdmins(groupId)
                ...
                if (lastAdmin !== undefined && cd - (Date.now() - lastAdmin) > 0) {
                    ...
                    await yakeys.sendTextWithMentions(from, txt)
                } else {
                    let txt = '╔══✪〘 *ADMINS* 〙✪══\n'
                    for (let i = 0; i < groupAdm.length; i++) {
                        txt += '╠➥'
            ...
            case prefix+'everyone':
                ...
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                const groupMem = await yakeys.getGroupMembers(groupId)
            ...
            case prefix+'groupicon':
                ...
                if (isMedia && isImage || isQuotedImage) {
                    ...
                    const mediaData = await decryptMedia(encryptMedia, uaOverride)
                    const imageBase64 = `data:${_mimetype};base64,${mediaData.toString('base64')}`
                    await yakeys.setGroupIcon(groupId, imageBase64)
                    await yakeys.sendText(from, ind.ok())
                } else {
                    await yakeys.reply(from, ind.wrongFormat(), id)
                }
            break
            case prefix+'antilink':
                ...
                if (!isBotGroupAdmins) return await yakeys.reply(from, ind.botNotAdmin(), id)
                if (ar[0] === 'enable') {
                    if (isDetectorOn) return await yakeys.reply(from, ind.detectorOnAlready(), id)
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    _antilink.push(groupId)
                    fs.writeFileSync('./database/group/antilink.json', JSON.stringify(_antilink))
                    await yakeys.reply(from, ind.detectorOn(name, formattedTitle), id)
                } else if (ar[0] === 'disable') {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    _antilink.splice(groupId, 1)
                    fs.writeFileSync('./database/group/antilink.json', JSON.stringify(_antilink))
                    await yakeys.reply(from, ind.detectorOff(), id)
                } else {
                    await yakeys.reply(from, ind.wrongFormat(), id)
                }
            break
            case prefix+'leveling':
                ...
                if (!isGroupAdmins) return await yakeys.reply(from, ind.adminOnly(), id)
                if (ar[0] === 'enable') {
                    if (isLevelingOn) return await yakeys.reply(from, ind.levelingOnAlready(), id)
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    _leveling.push(groupId)
                    fs.writeFileSync('./database/group/leveling.json', JSON.stringify(_leveling))
                    await yakeys.reply(from, ind.levelingOn(), id)
                } else if (ar[0] === 'disable') {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    _leveling.splice(groupId, 1)
                    fs.writeFileSync('./database/group/leveling.json', JSON.stringify(_leveling))
                    await yakeys.reply(from, ind.levelingOff(), id)
                } else {
                    await yakeys.reply(from, ind.wrongFormat(), id)
                }
            break
            case prefix+'welcome':
                ...
                if (!isGroupAdmins) return await yakeys.reply(from, ind.adminOnly(), id)
                if (ar[0] === 'enable') {
                    if (isWelcomeOn) return await yakeys.reply(from, ind.welcomeOnAlready(), id)
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    _welcome.push(groupId)
                    fs.writeFileSync('./database/group/welcome.json', JSON.stringify(_welcome))
                    await yakeys.reply(from, ind.welcomeOn(), id)
                } else if (ar[0] === 'disable') {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    _welcome.splice(groupId, 1)
                    fs.writeFileSync('./database/group/welcome.json', JSON.stringify(_welcome))
                    await yakeys.reply(from, ind.welcomeOff(), id)
                } else {
                    await yakeys.reply(from, ind.wrongFormat(), id)
                }
            break
            ...
            case prefix+'autostik':
                ...
                if (!isGroupAdmins) return await yakeys.reply(from, ind.adminOnly(), id)
                if (ar[0] === 'enable') {
                    if (isAutoStickerOn) return await yakeys.reply(from, ind.autoStikOnAlready(), id)
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    _autosticker.push(groupId)
                    fs.writeFileSync('./database/group/autosticker.json', JSON.stringify(_autosticker))
                    await yakeys.reply(from, ind.autoStikOn(), id)
                } else if (ar[0] === 'disable') {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    _autosticker.splice(groupId, 1)
                    fs.writeFileSync('./database/group/autosticker.json', JSON.stringify(_autosticker))
                    await yakeys.reply(from, ind.autoStikOff(), id)
                } else {
                    await yakeys.reply(from, ind.wrongFormat(), id)
                }
            break
            case prefix+'antinsfw':
                ...
                if (!isBotGroupAdmins) return await yakeys.reply(from, ind.botNotAdmin(), id)
                if (ar[0] === 'enable') {
                    if (isDetectorOn) return await yakeys.reply(from, ind.antiNsfwOnAlready(), id)
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    _antinsfw.push(groupId)
                    fs.writeFileSync('./database/group/antinsfw.json', JSON.stringify(_antinsfw))
                    await yakeys.reply(from, ind.antiNsfwOn(name, formattedTitle), id)
                } else if (ar[0] === 'disable') {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await yakeys.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    _antinsfw.splice(groupId, 1)
                    fs.writeFileSync('./database/group/antinsfw.json', JSON.stringify(_antinsfw))
                    await yakeys.reply(from, ind.antiNsfwOff(), id)
                } else {
                    await yakeys.reply(from, ind.wrongFormat(), id)
                }
            break
            ...
            case prefix+'block':
            case prefix+'blok':
                if (!isOwner) return await yakeys.reply(from, ind.ownerOnly(), id)
                if (mentionedJidList.length !== 0) {
                    for (let blok of mentionedJidList) {
                        if (blok === botNumber) return await yakeys.reply(from, ind.wrongFormat(), id)
                        await yakeys.contactBlock(blok)
                    }
                    await yakeys.reply(from, ind.doneOwner(), id)
                } else if (args.length === 1) {
                    await yakeys.contactBlock(args[0] + '@c.us')
                    await yakeys.reply(from, ind.doneOwner(), id)
                } else {
                    await yakeys.reply(from, ind.wrongFormat(), id)
                }
            ...
            case prefix+'unblok':
                ...
                if (mentionedJidList.length !== 0) {
                    for (let blok of mentionedJidList) {
                        if (blok === botNumber) return await yakeys.reply(from, ind.wrongFormat(), id)
                        await yakeys.contactUnblock(blok)
                    }
                    await yakeys.reply(from, ind.doneOwner(), id)
                } else if (args.length === 1) {
                    await yakeys.contactUnblock(args[0] + '@c.us')
                    await yakeys.reply(from, ind.doneOwner(), id)
                } else {
                    await yakeys.reply(from, ind.wrongFormat(), id)
                }
            ...
            case prefix+'bc':
                if (!isOwner) return await yakeys.reply(from, ind.ownerOnly(), id)
                if (!q) return await yakeys.reply(from, ind.emptyMess(), id)
                const chats = await yakeys.getAllChatIds()
                for (let bcs of chats) {
                    let cvk = await yakeys.getChatById(bcs)
                    if (!cvk.isReadOnly) await yakeys.sendText(bcs, `${q}\n\n- YakkoXCode (Kal)\n_Broadcasted message_`)
                }
                await yakeys.reply(from, ind.doneOwner(), id)
            break
            case prefix+'clearall':
                if (!isOwner) return await yakeys.reply(from, ind.ownerOnly(), id)
                const allChats = await yakeys.getAllChats()
                for (let delChats of allChats) {
                    await yakeys.deleteChat(delChats.id)
                }
                await yakeys.reply(from, ind.doneOwner(), id)
            break
            case prefix+'leaveall':
                if (!isOwner) return await yakeys.reply(from, ind.ownerOnly(), id)
                if (!q) return await yakeys.reply(from, ind.emptyMess(), id)
                const allGroup = await yakeys.getAllGroups()
                for (let gclist of allGroup) {
                    await yakeys.sendText(gclist.contact.id, q)
                    await yakeys.leaveGroup(gclist.contact.id)
                }
                await yakeys.reply(from, ind.doneOwner())
            break
            case prefix+'getses':
                if (!isOwner) return await yakeys.reply(from, ind.ownerOnly(), id)
                const ses = await yakeys.getSnapshot()
                await yakeys.sendFile(from, ses, 'session.png', ind.doneOwner())
            break
            case prefix+'ban':
                if (!isOwner) return await yakeys.reply(from, ind.ownerOnly(), id)
                if (ar[0] === 'add') {
                    if (mentionedJidList.length !== 0) {
                        for (let benet of mentionedJidList) {
                            if (benet === botNumber) return await yakeys.reply(from, ind.wrongFormat(), id)
                            _ban.push(benet)
            ...
            case prefix+'ev':
                ...
                try {
                    ...
                    await yakeys.sendText(from, evaled)
                } catch (err) {
                    console.error(err)
                    await yakeys.reply(from, err, id)
                }
            ...
            case prefix+'shutdown':
                if (!isOwner) return await yakeys.reply(from, ind.ownerOnly(), id)
                await yakeys.sendText(from, 'Shutdown Byee Owner❣️~ 👋')
                    .then(async () => await yakeys.kill())
            ...
            case prefix+'mute':
                ...
                if (!isGroupAdmins) return await yakeys.reply(from, ind.adminOnly(), id)
                if (ar[0] === 'enable') {
                    if (isMute) return await yakeys.reply(from, ind.muteChatOnAlready(), id)
                    _mute.push(groupId)
                    fs.writeFileSync('./database/bot/mute.json', JSON.stringify(_mute))
                    await yakeys.reply(from, ind.muteChatOn(), id)
                } else if (ar[0] === 'disable') {
                    _mute.splice(groupId, 1)
                    fs.writeFileSync('./database/bot/mute.json', JSON.stringify(_mute))
                    await yakeys.reply(from, ind.muteChatOff(), id)
                } else {
                    await yakeys.reply(from, ind.wrongFormat(), id)
                }
            break
            case prefix+'setname':
                if (!isOwner) return await yakeys.reply(from, ind.ownerOnly(), id)
                if (!q || q.length > 25) return await yakeys.reply(from, ind.wrongFormat(), id)
                await yakeys.setMyName(q)
                await yakeys.reply(from, ind.nameChanged(q), id)
            break
            case prefix+'grouplist':
                if (!isRegistered) return await yakeys.reply(from, ind.notRegistered(), id)
                const getGroups = await yakeys.getAllGroups()
                ...
                await yakeys.sendText(from, txtGc)
            break
            case prefix+'reset':
                if (!isOwner) return await yakeys.reply(from, ind.ownerOnly(), id)
                const reset = []
            ...
            default:
                if (isCmd) {
                    await yakeys.reply(from, ind.cmdNotFound(command), id)
                }
            break
        }
    } catch (err) {
